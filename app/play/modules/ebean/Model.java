...
public class Model {
	...
	@Transient
	private transient Tuple<Method, Method> _idGetSet;

	@Transient
	private transient boolean generateUUID = false;
	...
	private Tuple<Method, Method> _idAccessors() {
		if (_idGetSet == null) {
			try {
				...
				while (clazz != null) {
					for (Field f : clazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(javax.persistence.Id.class)) {
							PropertyDescriptor idProperty = new PropertyDescriptor(f.getName(), clazz);
							_idGetSet = new Tuple(idProperty.getReadMethod(), idProperty.getWriteMethod());

							// @UUID
							if (f.getType() == String.class && f.isAnnotationPresent(UUID.class)) {
								generateUUID = true;
							}
						}
					}
				...
				if (_idGetSet == null) {
					throw Helper.logWrap("No @javax.persistence.Id field found in class [" + this.getClass() + "]");
				}
			} catch (Exception e) {
				throw Helper.logWrap(e);
			}
		}
		return _idGetSet;
	}
	...
	private void _setUUID() {
		// NOTE!!!
		// _getId() must be called first, since "generateUUID" has not been assigned
		if (_getId() == null && generateUUID) {
