...
public class PlayAwareClassWriter extends ClassWriter {
	...
	protected String getCommonSuperClass(String type1, String type2) {
		try {
			// First put all super classes of type1, including type1 (starting
			// with type2 is equivalent)
			...
			// Then check type2 and each of it's super classes in sequence if it
			// is in the set
			// First match is the common superclass.
			s = type2;
			while (true) {
				if (superTypes1.contains(s))
					return s;
				s = getSuperType(s);
			}
		} catch (Exception e) {
	...
	private String getSuperType(String type) throws ClassNotFoundException {
		...
		try {
			return ac != null ? new ClassReader(ac.enhancedByteCode).getSuperName() : new ClassReader(type)
					.getSuperName();
		} catch (IOException e) {
